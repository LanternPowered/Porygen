 ...
 */
fun Vector2d.cross(vector: Vector2d) =
    this.y * vector.x - this.x * vector.y

fun min(first: Vector2d, second: Vector2d) =
    Vector2d(min(first.x, second.x), min(first.y, second.y))

fun max(first: Vector2d, second: Vector2d) =
    Vector2d(max(first.x, second.x), max(first.y, second.y))

fun min(first: Vector2i, second: Vector2i) =
    Vector2i(min(first.x, second.x), min(first.y, second.y))

fun max(first: Vector2i, second: Vector2i) =
